...
| Graficos            | recharts                         | 3.x           | Graficos SVG para React — donut charts e bar charts no dashboard (CR-019) |
| Lint (FE)           | ESLint + typescript-eslint + react-hooks | 10.x / 8.x / 7.x | Preset completo com rules-of-hooks: error — previne classe de bug do CR-034 (CR-035) |
| Testes (FE)         | Vitest + jsdom                   | 4.x           | Testes unitarios de utils e cliente HTTP; environment jsdom para localStorage/window (CR-039) |
| Backend             | Python + FastAPI                 | FastAPI 0.139 (starlette 1.3) | Framework async com validacao Pydantic nativa, Swagger auto (CR-042: bump de seguranca) |
...
| Autenticacao (JWT)  | python-jose[cryptography]        | 3.5+          | Criacao e validacao de JWT com HS256 (CR-002, ADR-015; CR-042: bump de seguranca) |
...
- **Status:** Aceito
- **Contexto:** Os dois servicos de IA (analise financeira F06 e importacao de extratos F07) foram escritos contra `claude-sonnet-4-20250514`, que a Anthropic marcou como depreciado. Trocar apenas a variavel `CLAUDE_MODEL` nao resolve: a geracao atual **rejeita com 400** os parametros de amostragem (`temperature`/`top_p`/`top_k`) que ambos enviavam, e liga raciocinio (*thinking*) por padrao — o que invalida a leitura posicional da resposta (`content[0].text`), ja que o primeiro bloco passa a ser de raciocinio.
...
| Deps de dev (pip) | Arquivo separado | `requirements-dev.txt` | `pytest==8.*` | CR-035: ferramentas de dev/teste fora da imagem de producao do Railway; CI instala `-r requirements-dev.txt` |
