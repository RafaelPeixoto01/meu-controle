...
├── backend/
│   ├── alembic/versions/       # Migrations 001..013 (aplicar antes de rodar o backend)
...
- CR-052: Upload assíncrono da importação (F07, item E-B do [roadmap v2](docs/F07-v2-roadmap-importacao.md)) — `POST /api/imports` responde **202** com o lote em `processando` e a extração roda em `BackgroundTasks` (in-process, sem broker); migration 011 (`erro_mensagem`); falha da IA passa a ficar registrada no lote; `GET /api/imports/{id}` vira canal de polling (3s) e `/pending` inclui `processando`; RN-047/048 e ADR-021; 9 testes backend + 6 Vitest; validado E2E via Playwright (concluido)
